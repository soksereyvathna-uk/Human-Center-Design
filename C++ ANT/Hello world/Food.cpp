...
void InsertF()
{
	InsertBox();
	InsertD();
	file.open("F7.bin", ios::out | ios::app);
	fc.Inputf();
...
void UpdateF()
{
	...
	int updatef;
	file.open("F7.bin", ios::in | ios::out);
	if(file.bad())
	{
		system("cls");
...
void OrderF()
{
	...
	int orderidf;
	bool isorderf = false;
	file.open("F7.bin", ios::in | ios::binary);
	file.open("Order7.bin", ios::out | ios::app | ios::binary);
	if(file.bad())
	{
		system("cls");
		...
		while(!file.eof())
		{
			if(orderidf == fc.barcodef)
			{
				...
				if(fc.qtyf >= fc.qtyorder)
				{
					...
					file.write((char*)&fc,sizeof(fc));
					file.write((char*)&fc,sizeof(fc));	//new file
											
					isorderf=true;
					gotoxy(90,40);cout << "--->>> Press Any key For Back";
					break;
				}
				if(fc.qtyf < fc.qtyorder)
				{					
					...
					getch();
					//ad.ClearText(94,37,40,8);
					//ad.ClearText(90,38,40,8);
					//ad.ClearText(125,35,6,8);		
					//goto h;	
				}
			}
			file.read((char*)&fc, sizeof(fc));
